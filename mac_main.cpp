...
int main(int argc, char** argv)
{
	...
	int 	opt;
	string working_dir = current_path();
	string git_dir = working_dir + "/.git";
	vector<string> exceptions;
	if(is_directory(filesystem::status(git_dir)))
	{
		exceptions.push_back(git_dir);
		for(auto& gfile : recursive_directory_iterator(git_dir))
		{
		...
		switch(opt)
		{
			...
			case 'e':
				...
				while(getline(e_file, estring))
				{
					...
					{
						exceptions.push_back(estring);
						if(is_directory(status(estring)))
						{	
							
							for(auto& Efile : recursive_directory_iterator(estring))
							{
			...
			case 'l':
				...
				for(const auto& lfile : directory_iterator(working_dir))
				{
					cout << "-> " <<lfile << endl;
				}
				return 0;
			case 'L':
				...
				for(const auto& lfile : directory_iterator(optarg))
				{
					cout << "-> " << lfile << endl;
				}
				return 0;
	...
	for(auto& file : recursive_directory_iterator(working_dir))
	{
